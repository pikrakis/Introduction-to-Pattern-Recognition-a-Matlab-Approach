function [MatchingProb,BestPath]=VitCoHMMsc(pi_init,A,B,O)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FUNCTION
%   [MatchingProb,BestPath]=VitCoHMMsc(pi_init,A,B,O)
...
if ~iscell(B)
    for i=1:N
        alpha(i,1)=pi_init(i)+log10(normpdf(O(:,1),B(1,i),B(2,i)));
    end
    ...
    for t=2:T
        for i=1:N
            temp=alpha(:,t-1)+A(:,i)+log10(normpdf(O(:,t),B(1,i),B(2,i)));
    ...
    for i=1:N
        alpha(i,1)=pi_init(i)+log10(mixt_value(B{1,i},B{2,i},B{3,i},O(:,1)));
    end    
    Pred(:,1)=zeros(N,1);
    ...
    for t=2:T
        for i=1:N
            temp=alpha(:,t-1)+A(:,i)+log10(mixt_value(B{1,i},B{2,i},B{3,i},O(:,t)));
            [alpha(i,t),ind]=max(temp);
            Pred(i,t)=ind+sqrt(-1)*(t-1);
        end
    end
